...
Advertiser: An entity that publishes advertisements on the platform. Advertisers also keep their advertisement
...
Bid Amount: Bid amount is the amount earned per advertisement by the

	platform from the advertiser. If the Bid amount is higher,
	then there are more chances of the advertisement getting served to the customer.

Advertisement Campaign: The Advertiser creates advertisement campaigns on the platform.

	The advertiser provides info such as URL, content_type (image, video), and bid per Ad.
	matching, and targeting attributes such as age, interest, and gender, etc., with the campaign.

System Constraints: The Platform will have some constraints and

	will be checked against all the advertisements before matching.

Advertisement Matching: The platform provides advertisements,

	comparing user details and attributes with all the active campaign requirements.
	It also compares the bid amount with the existing budget and checks for all the given system
	constraints for advertisement serving to find the best-matching advertisement.
	Once an advertisement is served, the budget is reduced from the advertiser’s account.
	An advertisement is served only when the bid amount is less than the existing budget.

flow ->

advertisement engine

		 -> list of users ( separately register with their attributes )
	     -> list of advertisers ( separately register with budget ammount )
		 -> let say all advertiser create compaign -> bid amount + attributes.
		 -> engine -> see all list of compaign against the user -> validate -> find perfect matching -> tell to user ??
...
// user <-> ad => view => max 10
// what is global level  -> overall ratelimiting right ?
...
func main() {
	ae := NewAdvertisementEngine()
	er := ae.AddAdvertiser("paytm")
	fmt.Println(er)
	er = ae.AddAdvertiserBudget("paytm", 100)
	fmt.Println(er)
	er = ae.AddUser("anubhav", 20, Male)
	fmt.Println(er)
	ae.CreateCampaign(1, 30, "hello world", 30, 10, []Gender{Male})
	for i := 0; i < 10; i++ {
		ad, err := ae.MatchAdvertisement("anubhav")
		fmt.Println(ad)
		fmt.Println(err)
	}
